...
We will investigate numerically the probability that a bootstrap sample of size n = 100 contains the jth observation. Here j = 4. We repeatedly create bootstrap samples, and each time we record whether or not the fourth observation is contained in the bootstrap sample.
...
```{r,echo=TRUE}
ggplot(aes(x = x, y = y), data = data_1) +
  geom_point()
#
```
...
```{r,echo=TRUE}
set.seed(19)
glm.fit <- glm(y ~ x, data = data_1)
cv.err <- cv.glm(data = data_1, glm.fit)
cv.err$delta
```
```{r}
set.seed(19)
...
~~~
When i = 5 has the smallest LOOCV error.
...
```{r,echo=TRUE}
attach(Default)
fit_1 = glm(default ~ income + balance, data = Default, family = "binomial")
fit_1
```
...
```{r,echo=TRUE}
fit_2 = glm(default ~ income + balance, data = Default, family = "binomial", subset = train)
fit_2
#
```
...
```{r,echo=TRUE}
pred_2 = predict(fit_2, Default[-train])
pred_2_b = ifelse(pred_2 > 0.5, "Yes", "No")
#
```
...
```{r,echo=TRUE}
mean(pred_2_b != Default[-train]$default)
#
```
...
validation set error is 0.0275
...
```{r,echo=TRUE}
set.seed(3)
train_3 <- sample(10000, 5000)
fit_3 = glm(default ~ income + balance, data = Default, family = "binomial", subset = train_3)
pred_3 = predict(fit_3, Default[-train_3])
pred_3_b = ifelse(pred_3 > 0.5, "Yes", "No")
mean(na.omit(pred_3_b != Default[-train_3]$default))
```
...
```{r,echo=TRUE}
fit_4 = glm(default ~ income + balance, data = Default, family = "binomial")
summary(fit_4)
```
